...
const Navbar = ({ terminalMode, setTerminalMode, uiType, setUiType }: Props) => {
  ...
  useEffect(() => {
    fetch("https://api.github.com/repos/nikhildhankar/portfolio")
      .then((res) => res.json())
      .then((data) => {
          ...
          >
            ...
            <div className="relative">
              <span className="font-black text-2xl tracking-tighter uppercase text-white flex items-baseline">
                NIKHIL<span className={`text-[17px] ml-0.5 transition-colors ${terminalMode ? "text-zinc-500 group-hover:text-green-500" : "text-zinc-500 group-hover:text-blue-500"}`}>DHANKAR</span>
              </span>
